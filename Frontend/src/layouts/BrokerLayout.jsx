...
import { 
  ...
  Menu, X, LogOut, Navigation, Star, User, Settings, Users, Percent, FileText
} from 'lucide-react';
...
export default function BrokerLayout({ children }) {
  ...
  const activeBroker = getMockData('brokers')[0]; // Lwazi Dlamini

  useEffect(() => {
    const notifs = getMockData('notifications') || {};
    if (notifs.broker) {
      setNotifications(notifs.broker);
    }
  }, [location]);
  ...
  const markAllRead = () => {
    ...
    allNotifs.broker = allNotifs.broker.map(n => ({ ...n, read: true }));
  ...
  const navItems = [
    ...
    { name: 'Leads Bidding', path: '/broker/leads', icon: Percent },
    { name: 'Assigned Loads', path: '/broker/assigned-loads', icon: Navigation },
    { name: 'My Shippers', path: '/broker/customers', icon: Users },
    { name: 'Commission Reports', path: '/broker/reports', icon: FileText }
  ];
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-slate-900 border-r border-slate-800 text-slate-300 shrink-0">
        <div className="h-16 flex items-center px-6 border-b border-slate-800 gap-2">
          ...
          <span className="font-extrabold text-xl tracking-tight text-white">
            Load<span className="text-indigo-500">Africa</span>
          </span>
          <span className="text-[10px] bg-indigo-500/20 text-indigo-400 font-semibold px-1.5 py-0.5 rounded border border-indigo-500/30">BROKER</span>
        </div>
        ...
        <div className="p-4 border-t border-slate-800">
          <div className="flex items-center gap-3 px-2 py-3 rounded-xl bg-slate-800/40 border border-slate-800">
            <img 
              src={activeBroker.avatar} 
              alt={activeBroker.name} 
              className="h-10 w-10 rounded-full border border-slate-705 object-cover"
            />
            <div className="min-w-0 flex-1 text-left">
              <p className="text-sm font-medium text-white truncate">{activeBroker.name}</p>
              <p className="text-xs text-indigo-455 text-indigo-400 truncate">Commissions: ${activeBroker.commissionEarned}</p>
            </div>
          </div>
          <button 
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex flex-col w-64 bg-slate-900 text-slate-300 transform transition-transform duration-300 ease-in-out lg:hidden ${
        ...
        <div className="h-16 flex items-center justify-between px-6 border-b border-slate-800">
          <div className="flex items-center gap-2">
            <Percent className="h-7 w-7 text-indigo-500" />
            <span className="font-extrabold text-xl text-white">Load<span className="text-indigo-500">Africa</span></span>
          </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 ${
                  isActive 
                    ? 'bg-indigo-650 text-white shadow-lg shadow-indigo-600/20 font-semibold' 
                    : 'hover:bg-slate-800/60 hover:text-white'
                }`}
              >
                <Icon className={`h-5 w-5 mr-3 ${isActive ? 'text-white' : 'text-slate-400'}`} />
        ...
        <div className="p-4 border-t border-slate-800">
          <div className="flex items-center gap-3 px-2 py-3 rounded-xl bg-slate-800/40 border border-slate-800">
            <img src={activeBroker.avatar} alt={activeBroker.name} className="h-10 w-10 rounded-full object-cover" />
            <div className="min-w-0 flex-1 text-left">
              <p className="text-sm font-medium text-white truncate">{activeBroker.name}</p>
              <p className="text-xs text-indigo-400 truncate">Commissions: ${activeBroker.commissionEarned}</p>
            </div>
          </div>
          <button 
            onClick={handleLogout}
        ...
        <header className="relative h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 lg:px-8 z-50 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:block">
              <h1 className="text-lg font-black text-slate-900 font-sans tracking-tight">Broker Panel</h1>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
                  ...
                  <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-xl border border-slate-150 py-2 z-30 transform origin-top-right transition-all">
                    <div className="px-4 py-2 border-b border-slate-100 flex items-center justify-between">
                      ...
                      {unreadCount > 0 && (
                        <button 
                          onClick={markAllRead}
                          className="text-xs text-indigo-650 text-indigo-600 font-medium"
                        >
                          Mark all read
                        </button>
                      )}
                      ...
                      {notifications.length === 0 ? (
                        ...
                        notifications.map((notif) => (
                          <div 
                            ...
                            onClick={() => {
                              const allNotifs = getMockData('notifications');
                              allNotifs.broker = allNotifs.broker.map(n => n.id === notif.id ? { ...n, read: true } : n);
                              saveMockData('notifications', allNotifs);
                              setNotifications(allNotifs.broker);
            ...
            <div className="relative">
              ...
              >
                ...
                />
                <span className="hidden md:block text-sm font-semibold text-slate-700">{activeBroker.name.split(' ')[0]}</span>
              </button>
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-slate-150 py-2 z-30 transform origin-top-right transition-all">
                    <div className="px-4 py-3 border-b border-slate-100 text-left">
                      <p className="text-sm font-bold text-slate-800">{activeBroker.name}</p>
                      <p className="text-xs text-slate-500 truncate">{activeBroker.email}</p>
                    </div>
                    <Link 
                      to="/broker/dashboard" 
                    ...
                    <button 
                      onClick={handleLogout}
                      className="flex items-center w-full text-left px-4 py-2.5 text-sm text-red-650 text-red-600 hover:bg-red-50 transition-colors"
                    >
