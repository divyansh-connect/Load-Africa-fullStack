...
export default function PlantOwners() {
  ...
  const fetchOwners = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const res = await adminService.getUsersByRole({ role: 'PLANT_OWNER', page, search });
      if (res.success) {
        setOwners(res.data);
      ...
      setError('Failed to load plant owners');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 font-medium">Manage Yellow Plant equipment owner applications. Total: {totalOwners}</p>
        </div>
      </div>
              ...
              {owners.map((owner) => (
                <tr key={owner.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <p className="font-medium text-slate-700">{owner.first_name ? `${owner.first_name} ${owner.last_name || ''}` : 'User'}</p>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      {owner.status === 'PENDING' && (
                        <>
                          <button onClick={() => handleAction(owner.id, 'ACTIVE')} className="p-1.5 text-slate-400 hover:text-green-500 hover:bg-green-50 rounded-lg transition-colors" title="Approve">
                      ...
                      {owner.status === 'ACTIVE' && (
                         <button onClick={() => handleAction(owner.id, 'SUSPENDED')} className="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Suspend">
                           <PauseCircle className="h-4 w-4" />
                         </button>
                      )}
                      {owner.status === 'SUSPENDED' && (
                         <button onClick={() => handleAction(owner.id, 'ACTIVE')} className="p-1.5 text-slate-400 hover:text-green-500 hover:bg-green-50 rounded-lg transition-colors" title="Reactivate">
                           <CheckCircle2 className="h-4 w-4" />
                         </button>
                      )}
